...
Write-Host "[Encore Flow] Autostart: pm2 resurrect..." -ForegroundColor Cyan
npx pm2 resurrect

$list = npx pm2 jlist 2>$null
if (-not $list -or $list -eq "[]") {
  Write-Host "[Encore Flow] Dump empty, starting prod ecosystem..." -ForegroundColor Yellow
  npx pm2 start ecosystem.prod.config.js --update-env
  npx pm2 save | Out-Null
}

Write-Host "[Encore Flow] Autostart done." -ForegroundColor Green
